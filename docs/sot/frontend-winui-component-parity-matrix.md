...
This file is a parity audit and working reference, not the product source of truth. The real source of truth for intended UI behavior, structure, and lifecycle remains the current frontend implementation. Use this matrix to track WinUI status against that implementation, not to override it.
...
- This file now includes explicit full outer-join inventories for frontend `components`, `hooks`, and `lib` seams versus the current WinUI app so future parity work starts from the real frontend source inventories.
