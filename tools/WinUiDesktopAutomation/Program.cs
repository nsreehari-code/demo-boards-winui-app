...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        try
        {
            ...
            if (options.RunSuite && options.HoldOpenDuration > TimeSpan.Zero)
            {
            ...
            Console.WriteLine(options.RunSuite
    ...
    private sealed record Options(bool ReuseRunning, TimeSpan Timeout, bool RunSuite, TimeSpan HoldOpenDuration)
    {
        public static Options Parse(string[] args)
        {
            ...
            for (int index = 0; index < args.Length; index++)
            {
                ...
                throw new ArgumentException($"Unknown argument '{arg}'. Supported flags: --reuse-running, --timeout-seconds <value>, --ui-only, --hold-open-seconds <value>.");
            }

            return new Options(reuseRunning, TimeSpan.FromSeconds(timeoutSeconds), runSuite, TimeSpan.FromSeconds(holdOpenSeconds));
        }
    }
}