...
public sealed class DemoBoardsRuntimeService : IAsyncDisposable
{
    ...
    private CancellationTokenSource? queuePumpCts;
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            StartQueuePump(listenerCts.Token);
    ...
    public async Task StopAsync()
    {
        ...
        if (queuePumpCts is not null)
        {
            queuePumpCts.Cancel();
        }

        if (queuePumpTask is not null)
        {
            try
            {
    ...
    private void StartQueuePump(CancellationToken cancellationToken)
    {
        int intervalMs = Math.Max(1, options.QueuePumpIntervalMs);
        queuePumpCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        queuePumpTask = Task.Run(() => RunQueuePumpLoopAsync(intervalMs, queuePumpCts.Token), queuePumpCts.Token);
    }

    private async Task RunQueuePumpLoopAsync(int intervalMs, CancellationToken cancellationToken)
    {
        ...
        while (await timer.WaitForNextTickAsync(cancellationToken).ConfigureAwait(false))
        {
            try
            {
                await jsHost.InvokeJsAsync("winuiPumpRuntimeQueues").ConfigureAwait(false);
            }
